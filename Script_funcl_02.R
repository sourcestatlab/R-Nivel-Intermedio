...
# Split con más de dos factores
vec <- rpois(10, 5)
fac1 <- gl(2, 5)
fac2 <- gl(5, 2)
inter <- interaction(fac1, fac2)
fac1
fac2
inter

# Split con más de dos factores
df <- data.frame(vec, fac1, fac2, inter)
df

# La interacción entre factores puede crear niveles vacíos.
str(split(x = vec, f = list(fac1, fac2)))

# Los niveles vacíos se reducen mediante drop = TRUE.
str(split(x = vec, f = list(fac1, fac2), drop = TRUE))
